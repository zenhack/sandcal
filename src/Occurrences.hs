...
zonedOCTimeToUTCFudge zot =
  let localTime = case octTime zot of
   ...
   in TZ.localTimeToUTCTZ (TZ.tzByLabel $ octZone zot) localTime
...
zonedEndTime defaultTz ev = case veDTEndDuration ev of
  ...
  Just (Right (DurationProp dur _)) ->
    let start = zonedStartTime defaultTz ev
     in case octTime start of
          LocalOCAtTime t ->
...
mergeManyOn f lists =
  case lists of
    ...
    [x] -> x
    _ ->
      let (xs, ys) = split lists
       in mergeOn f (mergeManyOn f xs) (mergeManyOn f ys)
  where
    split [] = ([], [])
    split [x] = ([x], [])
    split (x : y : zs) =
...
firstOccurrence lbl ev =
  ...
  let startPoint =
   ...
   in case eventOccurrences lbl startPoint ev of
        [] -> Nothing
        (oc : _) -> Just oc
...
expandFreq defaultTz viewStart ev freq interval =
  ...
  where
    ...
    expandSeconds toSeconds =
      let toNDF = toSeconds >>> fromIntegral >>> Time.secondsToNominalDiffTime
       in case octTime eventStart of
            LocalOCAllDay _ ->
            ...
            LocalOCAtTime localTime ->
              expandIndex $ \i ->
                let bump t =
                 ...
                 in (bump eventStart, bump eventEnd)
...
ruleOccurrences defaultTz start vevent recur =
  let unbounded = unboundedOccurrences defaultTz start vevent recur
   in case recurUntilCount recur of
        Nothing -> unbounded
