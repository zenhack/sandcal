...
labeledInput name attrs =
  let name' = H.toValue name
   in H.div ! A.class_ "labeledInput" $ do
...
postForm csrfKey cap attrs contents =
  let token = CSRF.makeCsrfToken csrfKey cap
   in H.form
        ! attrs
        ! A.method "post"
        ! A.action (H.toValue (CSRF.route cap))
        $ do
...
tzSelect label userTz =
  let addSelected :: TZ.TZLabel -> (H.Html -> H.Html) -> H.Html -> H.Html
   ...
   in H.select ! A.id (H.toValue label) ! A.name (H.toValue label) $
